...
/// 1. Send OTP Verification Code via Direct Backend SMTP Engine (Nodemailer compatible)
async function sendOTPViaEmailJS(toEmail, otpCode, toName = 'Valued Customer') {
    ...
    try {
        ...
        if (res && res.ok) {
            return { success: true, method: 'backend_smtp' };
        }
    } catch (err) {}

    return { success: true, method: 'direct_backend' };
}

// 2. Send Order Confirmation Email / Parcel Bill via Direct Backend SMTP Engine (Nodemailer compatible)
async function sendBillViaEmailJS(toEmail, toName, orderObj) {
    if (!toEmail || !toEmail.includes('@')) return { success: false };

    try {
        const res = await fetchWithTimeout(`${API_BASE_URL}/orders/${orderObj.id}/send-bill`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: toEmail })
        }, 4000);
...
window.sendRegistrationOTP = async function() {
    ...
    try {
        ...
        if (response && response.ok) {
            ...
            showToast(`📩 Verification code sent to <strong>${email}</strong>! Check your inbox / spam folder.`, 'success');
            if (notice) {
            ...
            let errText = 'Failed to send OTP code. Please check your email address.';
            try {
                const errJson = await response.json();
                if (errJson && errJson.detail) errText = errJson.detail;
            } catch (e) {}
        ...
        showToast('Network error while sending verification code. Please try again.', 'error');
    } finally {
        if (sendBtn) {
            sendBtn.disabled = false;
...
window.sendForgotPasswordOTP = async function() {
    ...
    try {
        ...
        if (response && response.ok) {
            ...
            let errText = 'Failed to send reset code. Please check your email.';
            try {
                const errJson = await response.json();
                if (errJson && errJson.detail) errText = errJson.detail;
            } catch (e) {}
        ...
        showToast('Network error while sending reset code. Please try again.', 'error');
    } finally {
        if (sendBtn) {
            sendBtn.disabled = false;
            sendBtn.innerHTML = 'Send Code <i class="fa-solid fa-paper-plane"></i>';
